...
- `stop_reason` は
  `Literal["completed","max_iterations","budget_exceeded","denied"]` — どの
  ガードレールで止まったかが型レベルで判別可能。`final_output` は `str | None`
  （ガードレール発火で未完了時は `None`）。
...
- 正常完了 + 4契約違反系（許可リスト違反→継続 / 承認拒否 / 予算超過 /
...
- 本パターンは唯一の「Agent」型で **OWASP Agentic AI Top 10 の主戦場**。固有
  ...
  - 過剰エージェンシー / Insecure Tool Use → `allowed_tools` 外は**実行せず
    拒否 observation を feedback してループ継続**（Req 6.4、per-call refusal）。
