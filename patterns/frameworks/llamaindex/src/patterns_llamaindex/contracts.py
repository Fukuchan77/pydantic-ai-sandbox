...
The normative copy lives in ``patterns/routing/README.md`` and
``patterns/orchestrator-workers/README.md``; each lane carries an
identical duplicate (plan AD-3) so the independent uv projects never
import across lane boundaries (NFR-3). Edit all three lanes (and the
README normative copy) together.
"""
