"""Hermetic-guard fixture for the beeai unit suite (X-2).

Every unit test runs network-free: the ``ScriptedChatModel`` fake never opens
a socket. The autouse ``block_network`` fixture turns any accidental internet
reach -- an un-faked chat model, an OTLP export, a stray HTTP client -- into a
loud ``NetworkReachError`` instead of silent I/O.
