...
- 脆弱モジュール `src/json_repair/schema_repair.py` は upstream commit `9a27d7c8`
  ...
  `v0.55.2` に**不在**、`v0.56.0` 以降に存在。よって真の脆弱範囲は **0.56.0–0.59.10**
  であり、advisory の `< 0.60.1` は下限（`>= 0.56.0`）が欠落した**過大申告**である。
...
正しい再評価トリガは「`json-repair` のロック版が **0.56.0–0.59.10** に入った場合」であり、
