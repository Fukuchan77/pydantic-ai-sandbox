...
のを上限ピンで塞ぐ運用を記録する。抑止が「advisory を無視する」のに対し、これは
「脆弱版が選ばれないよう解空間を狭める」措置であり、pip-audit は green のままとなる。

**事象**: ルート `pyproject.toml` に対する `uv lock --upgrade` が litellm を
**1.97.0 → 1.83.0 へダウングレード**した。原因は次の上流制約の衝突である。

- litellm は openai 3.x を未採用 — 1.83.1 以降・最新 1.97.0 まで、全リリースが
  `openai==2.x` または `openai<3.0.0,>=2.20.0` を宣言する。
- pydantic-ai-slim は `openai` extra のフロアを **2.32.0 で `openai>=3.0.0` へ引き上げた**。

両者を同時に満たす唯一の解が litellm 1.83.0（`openai>=2.8.0` に上限が無い唯一の
リリース。上流の宣言漏れであって互換性の表明ではない）であり、解決器はそこへ後退した。

**実測**: その解に対する pip-audit は litellm 1.83.0 に **11 件の既知脆弱性**を報告した
（PYSEC-2026-388 / 391 / 2598 / 2599 / 2600 / 2601 / 2602 / 3476 / 3477 / 3479、
修正は 1.83.7–1.84.0 に分散）。すなわち「最新化」がそのまま**脆弱性の新規導入**になる
構図であり、ロック更新を無検証で受け入れてはならない実例である。

**措置**: ルートの `litellm` extra と dev グループの双方へ `openai<3.0.0` を宣言した
（`pyproject.toml` の ADR-2）。これにより litellm は最新 1.97.0（脆弱性なし）に留まり、
pydantic-ai-slim は openai 2.x を許容する **2.31.x 系**に据え置かれる。dev グループにも
重複して宣言するのは、extra を含まない dev のみの解決が再び 1.83.0 へ後退しうるためである。

**撤去条件**: litellm が `openai>=3` を宣言するリリースを出した時点で上限ピンを削除し、
pydantic-ai-slim を 2.32.0 以降へ引き上げる。それまでルート単独では slim 2.32+ を採用しない。

**レーンへの非波及**: 本上限ピンは**ルート限定**である。litellm を持たない
`patterns/frameworks/pydantic-ai` / `sse` / `deep-research` / `hitl` の4レーンは
制約を受けず、pydantic-ai-slim 2.33.0 + openai 3.3.1 を採用する（全レーン
pip-audit green・テスト green を実測確認済み）。
