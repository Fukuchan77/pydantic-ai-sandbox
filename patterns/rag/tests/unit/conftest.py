"""Hermetic-guard fixture for the RAG unit suite (Spec 007-2b Req 6.1, X-2).

Every unit test runs network-free: the real ``HybridChunker`` plus the
deterministic ``HashEmbedding`` / ``ScriptedLLM`` fakes never open a socket.
The autouse ``block_network`` fixture turns any accidental internet reach --
an un-faked embedding/LLM, a tokenizer download, an OTLP export -- into a loud
