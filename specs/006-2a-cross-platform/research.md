...
- **Findings**: **import 方式 + contracts パッケージ内設置**を採用。
  ...
  (a) 各 `patterns/<pattern>/README.md` の **`# [contract]` マーカー付き**
  ```python fenced block（signature とは分離された `ast.parse` 可能な純 Python。
  現行 README の正本ブロックは `model/llm` 疑似構文の signature が混在し
  `ast.parse` 不能のため再整形が必要）を `ast` で
