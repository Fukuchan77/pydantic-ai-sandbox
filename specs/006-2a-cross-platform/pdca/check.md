...
| autonomous-agent: 4ガードレールを契約レベルで全レーン共通化、`stop_reason` を `Literal` 4値固定 | `max_iterations`/`allowed_tools`/`approval_hook`/`budget` を3レーン同一シグネチャで実装、`_budget_spent` シーム1点化で予算会計を決定論化 | ✅ |
...
- **ガードレール境界の非対称設計（spec 非明示の設計判断）**: `max_iterations`/`denied`/`budget_exceeded` は**ループ停止**、`allowed_tools` 違反のみ**per-call refusal + 継続**。根拠は `stop_reason` 語彙に "forbidden" 系が無い非対称性。3レーン完全同一に確定。
