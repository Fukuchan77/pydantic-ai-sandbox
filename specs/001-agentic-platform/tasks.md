...
- 全 member が stub の場合は `RuntimeError` (Req 4.5)、混在ケース (`ollama,watsonx` 等) では stub を **silent drop** して real provider のみで `FallbackModel` を構築する設計を採用。**PDCA Act (2026-05-24) で正式仕様として承認** (`spec.json` Amendment 2026-05-24T22:30:00Z 参照): ユーザの `FALLBACK_ORDER` 相対順を尊重しつつ stub 由来の `NotImplementedError` を `/chat` まで遅延させない (Plan §2.4 "これにより NotImplementedError を /chat 呼び出し時まで遅延させない" の意図と整合)。`test_build_fallback_skips_stub_members_when_real_members_remain` がこの挙動を契約として固定する。
