...
- Q3: 定期セキュリティスキャンのツールセットは？ → A: **pip-audit + ruff `S` (flake8-bandit 移植) + gitleaks**。pip-audit=依存 CVE（PyPI Advisory DB）、ruff `S` ルール群=Python コード脆弱性 (assert/exec/弱ハッシュ/SSL/subprocess/SQL injection 等を網羅)、gitleaks=シークレット漏洩、の三層を OSS で構成。CI (GitHub Actions など) で週次実行 + pre-commit に軽量チェックを組み込み、LiteLLM 系サプライチェーン事故にも備える。**注**: 当初 Q3 では bandit 単体導入を回答していたが、`pyproject.toml` の ruff 設定に既に `S` ルールが選択されており、bandit を別途インストールしても Py3.14 strict プロジェクトでは追加検出価値が無いと実装段で判明したため、ruff `S` で一本化した (2026-05-24 amendment / specs/001-agentic-platform/pdca/do.md 参照)。
- Q4: pre-commit で自動実行する品質ゲート範囲は？ → A: **lint+format+型チェック+軽量セキュリティ**。`ruff check` (`S` ルール = flake8-bandit 含む) / `ruff format --check` / `pyright` / `gitleaks` を pre-commit で実行し、`pytest` / `pip-audit` は CI 側に寄せる。コミット速度を維持しつつ重要ゲートは確保。`pre-commit run --hook-stage manual` で全件実行も可能にする。
...
- 8.3 [U] The default stage SHALL NOT include `pytest` or `pip-audit`; those SHALL be configured under `--hook-stage manual` for opt-in full sweeps. (Note: the original Q3 wording also listed `bandit` here, but bandit-equivalent checks are now executed via ruff's `S` rules inside `ruff check` on the default stage; see Clarification Q3 amendment.)
...
- 9.2 [U] CI SHALL execute Python static security scanning against `src/` on every push. This requirement is satisfied by `ruff check .` running the `S` (flake8-bandit) rule set inside `mise run check` on every CI lane; standalone `bandit` is intentionally not added (see Clarification Q3 amendment / research.md "bandit 非導入の根拠").
