...
| `pyproject.toml` (修正)                    | `pydantic-ai-slim[openai]` 追加、dev に `pip-audit`, `pytest-cov` 追加。`[tool.coverage.report]` に `fail_under = 0` baseline を設定。bandit は ruff `S` ルール群 (flake8-bandit 移植) で完全代替するため非導入                                                                                                                                                                                               | 6.1, 7.7, 9.1, 9.2 |
...
| `.pre-commit-config.yaml`                  | `ruff check` (flake8-bandit `S` 含む), `ruff format --check`, `pyright`, `gitleaks`, `forbid-hardcoded-model-ids` を default stage / `pytest`, `pip-audit` を `manual` stage。bandit 単体は default の ruff `S` で常時実行されるため manual stage に再掲しない                                                                                                                                                | 1.5, 8.1, 8.2, 8.3 |
| `.github/workflows/ci.yml`                 | push/PR で `mise run check` + `mise run pre-commit:manual` を実行                                                                                                                                                                                                                                                                                                                                             | 7.1-7.4, 8.4       |
| `.github/workflows/security.yml`           | push + 週次 cron で `pip-audit` / `gitleaks` を実行、HIGH/CRITICAL で fail。Python コード脆弱性 (Req 9.2) は `mise run check` 経由の `ruff check .` (`S` ルール) が CI 全レーンで実行されるためこのワークフローでは扱わない                                                                                                                                                                                   | 9.1-9.4            |
...
- `pyproject.toml` 上の `pre-commit / pip-audit / pytest-cov` 追加は `tasks.md` の早期タスクに含める必要がある (bandit は ruff `S` で代替するため非追加)。
