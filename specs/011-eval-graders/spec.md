...
> **Status: Tasks Generated（EARS 正規化済・plan/tasks/spec.json 整備済、tasks 承認待ち）**。改善提案 P4 の起票。
> 出典 `specs/best-practices-review/improvement-plan.md` P4。4 論点は IBM/Anthropic/Google/AWS の公式指針に
> 照らし CONFIRMED（→ ADR-1〜4）。
...
本 spec は outcome+behavior グレーダを契約(多軸スコアの `GradeReport` 相当)として `contracts/` に追加し、
...
### Session 2026-06-25（/sdd-init による正本化）

- Q: 共有グレーダ契約のクラス名をどれにするか（draft 内で `AuditReport` 相当 と `GradeReport` 相当 が混在）→ A: **`GradeReport`**。Overview / Requirements / Glossary が一貫して使う名称に統一し、Project Description の `AuditReport` 表記を `GradeReport` へ訂正。`AuditReport` は本リポジトリの別概念(監査レポート)との混同を避ける。
...
- **Unknown / `"unknown"`**: 証拠不足で数値採点できない軸を表す概念を本書では `Unknown`(大文字)と書き、契約上の `Rating` Literal 値としては `"unknown"`(小文字)で表現する(同一概念の散文表記と型値の対応)。

## Requirements（EARS）

> EARS 規約: 主語は **「eval-graders 契約基盤」**（共有グレーダ契約 + その正本 README + ドリフトテスト + 各パターン eval からの参照を束ねる subsystem）に統一する（コンポーネント名を切り替えない）。`SHALL` を必須挙動に用い、トリガは `WHEN` / 状態は `WHILE` / 異常系は `IF…THEN` / 機能ゲートは `WHERE` で表す。1 受け入れ条件 = 1 検証可能挙動。

### Requirement 1: 共有グレーダ契約（ADR-1, ADR-2）

outcome+behavior の多軸スコアを単一契約に固定し、ランタイムゲートと eval グレーダの定義ドリフトを 1 点で防ぐ。

1.1 THE eval-graders 契約基盤 SHALL outcome+behavior の多軸スコアを表す単一契約 `GradeReport` を `patterns_contracts` の単一実体として提供する。
1.2 THE eval-graders 契約基盤 SHALL `GradeReport` 内で outcome 軸と behavior 軸を分離して保持し、各軸スコアを離散 1–5 rating + 証拠不足を表す `Unknown` + 根拠 rationale で表現する。
1.3 THE eval-graders 契約基盤 SHALL 最終集約スコアを partial credit を許す float として表現する。
1.4 THE eval-graders 契約基盤 SHALL 各 rating 段階（1–5）の意味を rubric 文言で定義する（Google Vertex の rating_rubric 方式）。
1.5 IF いずれかの軸スコアの rationale が空であるとき、THEN THE eval-graders 契約基盤 SHALL `GradeReport` の構築を拒否して loud-fail する（silent empty 禁止）。
1.6 THE eval-graders 契約基盤 SHALL グレーダ契約の正本を新規横断 README `patterns/EVAL-GRADERS.md` に所有させ、`_README_PATHS` へ登録のうえ 正本＝パッケージ実体の一致を `test_contract_drift.py` で検証する。

### Requirement 2: 複数パターンからの参照（ADR-4）

3 パターンが同一契約を参照することで単一ソース化を達成し、既存ランタイム契約とは併存させる。

2.1 WHEN evaluator-optimizer / deep-research / autonomous-agent の eval が採点結果を構築するとき、THE eval-graders 契約基盤 SHALL いずれのパターンからも同一の `GradeReport` 契約を import して構築できるようにする。
2.2 THE eval-graders 契約基盤 SHALL 既存ランタイム契約 `OptimizationResult` / `ResearchReport` / `AgentRunResult` を置換せず後方互換を維持し、`GradeReport` をランタイム収束ゲートと併存する別レイヤ（オフライン/CI 採点）として位置づける。
2.3 WHEN deep-research の成果を採点するとき、THE eval-graders 契約基盤 SHALL spec 010 で追加された `Finding.notes`（`ResearchNote` の distill 済みノート）を採点対象に含める。
2.4 IF `ResearchNote.key_point` が空または低信号（distill は空 `SearchResult.snippet` から空 key point を生成しうる）であるとき、THEN THE eval-graders 契約基盤 SHALL behavior/faithfulness 軸を silent にスコアせず `Unknown`（証拠不足、ADR-2）へマップする。

### Requirement 3: 独立 judge と self-eval バイアス回避（ADR-3）

独立性は実装規律で担保し、契約は純データ + judge 最小メタに限定する（型による誤った安全感を与えない）。

3.1 THE eval-graders 契約基盤 SHALL 採点を被評価系と分離した judge シーム（注入）で行い、self-eval を構造的に避ける。
3.2 WHERE オフライン決定論フェイク judge が注入されている場合、THE eval-graders 契約基盤 SHALL ネットワーク I/O なしで採点形状を hermetic に検証できるようにする。
3.3 THE eval-graders 契約基盤 SHALL 契約を純データ（軸スコア + rationale + 集約）+ judge 出自の最小メタ（`judge_id` 等の任意フィールド）に限定し、self-eval 禁止フラグ等の規律を型制約に含めない。

### Requirement 4: テスト

決定論フェイクによるオフライン hermetic 検証と既存ドリフト/カバレッジ水準を維持する。

4.1 THE eval-graders 契約基盤 SHALL 全グレーディング unit テストをネットワーク I/O ゼロ（決定論フェイク judge）で実行する。
4.2 THE eval-graders 契約基盤 SHALL 共有グレーダ契約が 3 パターンの eval から参照されることを検証する。
4.3 THE eval-graders 契約基盤 SHALL 契約ドリフトテストを緑に維持する。

### Requirement 5: ドキュメント

単一ソース化を解説とベストプラクティス検証へ反映する。

5.1 WHEN 本 spec が完了したとき、THE eval-graders 契約基盤 SHALL `patterns/EVAL-GRADERS.md` に正本を置き、各パターン README の評価節へ参照を追記する。
5.2 WHEN 本 spec が完了したとき、THE eval-graders 契約基盤 SHALL `specs/best-practices-review/verification.md` の観点6（評価）へ単一ソース化を反映する。
