...
- [ ] 3.1 失敗 eval テスト `test_eval_graders_deep_research.py` を先行作成する（ネットワーク I/O ゼロ）。フェイク `Judge[ResearchReport]` で `ResearchReport`（`Finding.notes` 含む）を採点し `GradeReport` 構築形状を検証、純粋ヘルパ `faithfulness_rating_for(notes) -> Rating` へ空 `key_point` / 非空 `key_point` を直接与えて `Unknown` 分岐を検証する。**赤を確認する。**
  _Boundary:_ `patterns/deep-research/tests/unit/test_eval_graders_deep_research.py`
  _Depends:_ 1.2
  _Requirements:_ 2.3, 2.4, 4.2
- [ ] 3.2 `tests/support/model_fakes.py` に決定論フェイク `Judge[ResearchReport]` と純粋ヘルパ `faithfulness_rating_for(notes) -> Rating`（空/低信号 `key_point` → `"unknown"`）を実装し、テストを緑化する。フェイクはヘルパを呼ぶ（台本焼き込みを避ける）。
