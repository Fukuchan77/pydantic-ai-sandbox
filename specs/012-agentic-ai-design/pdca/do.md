...
## Task 9.1 — `patterns/README.md` 索引への hitl レーン追加

**日付**: 2026-07-12
**Boundary**: `patterns/README.md`, `docs/README.md`
**Requirements**: 13.1, 13.2

### 実装(非 TDD — ドキュメント索引のみのタスク、tasks.md に「赤を確認する」記載なし。
Task 7.2/8.2 と同型の config/doc-only 追加)

`patterns/README.md` の「応用レイヤー」節群(RAG → SSE → Deep Research)の末尾、
「横断レイヤー(評価グレーダ)」節の直前に「応用レイヤー(HITL 停止・承認・再開
ハーネス)」節を新設。既存3節と同じ構成(導入文 → 表 → blockquote)を踏襲し:

- 契約(`ActionType` / `ResolutionAction` / `SupportOutput`)は正本
  `patterns/hitl/README.md` に既にある(Task 2.1)ため、フィールド定義を
  再宣言せず、リンクのみで参照(one-README 不変条件、Req 13 の設計ノートも
  同様にリンク先へ委譲)。
- Durable Execution(Temporal/DBOS/Prefect、Restate は Restate 側 SDK)と
  セキュリティ設計ノート(v1 併用時 `>=1.99.0`、`message_history`/URL の SSRF、
  `safe_download`)は 013 へのポインタとともに一文で言及し、詳細は hitl/README.md
  へ誘導(R13.1/13.2 の記述はすでに正本にあるため、索引側は要約 + リンクのみ)。

`docs/README.md`(実装ベストプラクティスガイド索引)への追記は**行わなかった**:
同ファイルのスコープは `specs/best-practices-review/` の検証結果を既存コードの
拡張シームへ適用した「ガイド」(tool-design.md / context-engineering.md)に限定
されており、HITL は best-practices-review 由来のガイドではなく独立パターンレーン
そのもの(patterns/README.md の応用レイヤー表がその正しい索引)。tasks.md の
「必要に応じ」の条件が本タスクでは不成立と判断した。

### VERIFY(Task 自身のスコープ)

| Gate | Cmd | 結果 |
|---|---|---|
| 索引反映確認(grep) | `grep -n "hitl" patterns/README.md` | HITL 節の見出し・表行・blockquote 内 3 箇所で参照(新規) |
| 契約ドリフト回帰 | `mise run patterns:check`(全 lane) | lint/format/typecheck/test 全緑、`patterns/contracts` の
`test_contract_drift.py` 4 passed(既存 `_README_PATHS["hitl"]` 登録に変化なし)、hitl レーン
30 passed / 2 skipped(カバレッジ 100%) |

ドキュメント索引のみの変更でソース・契約に触れていないため、既存 lint/typecheck/test の
全緑維持が本タスクの十分な検証範囲(コード変更ゼロ、drift 対象は per-lane README ⇔
contracts パッケージの一致であり、top-level 索引はそのスコープ外 — 既存 rag/sse/deep-research
の索引追加時も同様に専用テストは存在しない)。

### 学び

- top-level `patterns/README.md` の応用レイヤー索引には対応する自動テストが存在しない
  (drift テストは per-lane README ⇔ contracts パッケージのみを見る)。RAG/SSE/Deep
  Research 追加時からの既存パターンであり、本タスクもその慣行を踏襲した。
- 「必要に応じ」のような条件付き記載のタスクでは、条件不成立の判断とその根拠を明記して
  おくことが重要 — 何もしなかった理由が後から追跡できないと、レビューで「見落とし」と
  誤認されるリスクがある。
- Section 9(ドキュメント同期)完了。tasks.md 全 9 セクション完了。
