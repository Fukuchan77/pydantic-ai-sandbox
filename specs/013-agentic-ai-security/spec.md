...
- Q: 「012 の spec / 実装には手を入れない」(上記)と、本 spec の実装が 012 の `store.py`/`harness.py`/`app.py` を書き換える事実はどう整合するか? → A:
  [gap-analysis.md](gap-analysis.md) が正本の再スコープ。012 は 013 の tasks/plan 生成より後にマージされたため、本当に新規のロジックは
  R2(状態機械)/R3(監査証跡)のみで、それらは 012 が既に持つ `store.py`/`harness.py` への**加算的拡張**として実装される
  ("手を入れない" は「012 の要件・設計判断を覗さない」の意であり、「ファイルを一切変更しない」の意ではない)。
