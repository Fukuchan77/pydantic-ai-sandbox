...
`SessionStore` に状態機械(`pending → in_flight → consumed`)を足すのが最小差分。
`in_flight` は `claim()` が取得する実行中ロックで、`await harness.resume()` を跨いで
並行 `/resume` の二重実行を防ぐ(plan.md H-1 対応)。
...
`SessionRecord.state: Literal["pending", "in_flight", "consumed"]` + pending `tool_call_id`
集合を保持。`claim()` は `pending` のときのみ成功し**同期的に** `pending → in_flight` へ
遷移(並行 `/resume` 先勝ち)、終端 `consume()` → `consumed`、再 defer `settle_pending()`
→ `pending`、409 `release()` → `pending`。

| 事象 | HTTP |
|---|---|
| 未知 / `in_flight` / consumed / 失効 session | 404(存在秘匿 — 理由は本文に書かない、R1.2, R2.1) |
| 判断対象の `tool_call_id` が pending 集合外 | 409(`claim()` 後・`harness.resume()` 前に検証、`release()` で pending 復元、何も実行しない、R2.3) |
