...
### 追記（2026-06-15, 続）— P2 が rag レーンの hidden never-green を炙り出した

P1/P2 マージ後の post-merge マトリクス（main）で、**5 レーン中 4 つが green**（beeai /
pydantic-ai / sse、そして**初めて end-to-end 成功した llamaindex**）。唯一 **rag** が失敗。
原因はジョブ予算でもインフラでもなく、**実テストの OOM**：

```
ollama ResponseError: insufficient memory (attempted to allocate 20480.00 MB)
  failed to allocate buffer for kv cache;  options: { num_ctx: 131072 }
```

llama-index の `Ollama` が `context_window` 未指定だとモデル最大コンテキスト
（granite4.1 = 131072）を `num_ctx` として要求し、~20GB の KV cache で CPU ランナーが OOM。
これは llamaindex 根本原因 #4（`context_window=-1` OOM）と**同型のバグが rag レーンにも
潜在**していたもの。rag の実機 e2e がレーン分離で初めて生成段まで到達して露見した
（= もう一つの hidden never-green）。

- 措置: rag の `_ollama_llm()` に `context_window=8192` + `num_predict=512` +
  `request_timeout=1200.0` を設定（llamaindex の修正と同型）— 恒久採用。
- 含意: **P2 の分離設計が機能した証左**。集約単一ジョブのままなら、この失敗は再び末尾レーン
  のログに埋もれていた。次の構造的対策として **P5（多層タイムアウト/コンテキスト上限を 1 箇所
  に定数化）** の優先度を引き上げる — レーン毎に同じ OOM を踏み直さないため。
