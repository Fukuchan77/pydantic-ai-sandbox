...
### P4 — lock 衛生（未着手・beta 採用を前提に再設計）

**前提（重要）**: 一部レーンは pre-release を**意図的に**採用している。一律禁止は不可。
- `pydantic-ai`: `pydantic-ai-slim[openai]>=2.0.0b6`（v2 beta）→ `[tool.uv] prerelease = "allow"` で明示 opt-in。
- `sse`: dev closure が `pydantic-graph` の pre-release に依存 → 同じく `prerelease = "allow"`。
- `beeai`: `beeai-framework==0.1.39`（0.1.81 等の 0.x も同様）は **PEP440 上は通常リリース**
  （`aN/bN/rcN/devN` ではない）→ pre-release ポリシーの対象外。
- 区別すべきは **非意図の transitive pre-release**（root cause #3 の `httpx 1.0.dev3`、
  llamaindex＝既定 disallow レーン）だけ。意図的 beta は正当として通す。

**方針**:
- CI に `uv lock --check`（`--locked`）を**全レーン**追加 → lock ドリフト（pyproject と lock の
  不一致）を検知。意図的 pre-release は既に lock に固定済みなので競合しない。
- 「許可／不許可」は**レーン単位の `[tool.uv] prerelease`** を単一の宣言的真実源とする
  （allow = pydantic-ai / sse、既定 disallow = それ以外）。既定 disallow が非意図の
  transitive pre-release を構造的に防ぐ（httpx dev の再発防止）。
- （任意）**pre-release allowlist ガード**: lock 内の真の PEP440 pre-release が既知の
  allowlist（`pydantic-ai*` / `pydantic-graph` 等）に限られることを検証。allow レーンへの
  想定外 beta 混入や、disallow レーンへの dev 混入を検知。beeai 0.x は pre-release でないため不問。
