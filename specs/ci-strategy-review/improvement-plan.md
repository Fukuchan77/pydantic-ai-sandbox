...
- ✅ `mise` / `jdx/mise-action` の pin（root cause #1 の 404 flaky 解消、2026-06-15）。
  全 5 ワークフロー（ci / integration-ollama / integration-watsonx /
  patterns-integration-ollama / security）の `jdx/mise-action` を **可変 `@v2` から
  immutable SHA（`dba1968…` = v4.1.0）へ**固定し、`version: 2026.6.10` で **mise バイナリも
  固定**。これで「latest の mise アセットを毎回取得 →一時 404」を構造的に解消。
- ◐ nightly の総時間圧縮（要判断・現状は十分）:
  - **P2 のレーン並列化で主要な圧縮は達成済み**（旧: 単一ジョブ逐次で 45 分予算超過 → 現: 並列で
    wall-clock ~29 分、各レーン 60/90 分予算内）。nightly としては十分で予算逼迫なし。
  - さらなる圧縮レバーはトレードオフ:
    - 最長レーンは **pydantic-ai（~29 分）**。OpenAI 互換経路で generation cap 未設定のため、
      短縮には `ModelSettings(max_tokens)` 配線が要る（green レーンへ変更・API 依存）。
    - `LIVE_MAX_TOKENS` 引き下げ（512→256）は beeai/llamaindex/rag のみ短縮（pydantic-ai は不変）
      で wall-clock 寄与は小。かつ rag/構造化出力の truncation リスク（直近で安定化したばかり）。
    - 軽量/量子化モデルが最大効果だが、モデル選定は本計画の Open Questions（容量計測後に決定）。
  - → 安定性を優先し、積極的な圧縮は計測・判断の上で実施（次イテレーション）。
...
- [x] `mise` 取得起因の flaky 失敗が再発しない（P5: action SHA + mise version を固定）。
- [~] nightly 総時間圧縮: P2 並列化で達成済み（~29分）。さらなる圧縮はトレードオフのため要判断。
...
- **P5 実装済み（2026-06-15）**: 実機ノブを `patterns_contracts.live_ollama` に定数集約／
  キャッシュ競合修正／`mise`・`jdx/mise-action` の固定（404 flaky 解消）。nightly 総時間圧縮は
  P2 並列化で実用上達成（~29分）。さらなる圧縮は安定性とのトレードオフのため計測・判断待ち。
